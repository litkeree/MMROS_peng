...
namespace dmce {
		...
		: NodeServer(nh, robotId), timeSinceLastMapCheck_(0)
	{
		...
		} else if (plannerType == "seqtargets") {
			planner_ = std::make_unique<SequentialTargetsPlanner>(robotDiameter_);

		} else if (plannerType == "objectsearch") {
    		planner_ = std::make_unique<ObjectSearchPlanner>(robotDiameter_);
	...
	void PlannerServer::publishPlan_(const dmce_msgs::RobotPlan& planMsg) {   //箭头
	...
	bool PlannerServer::isMapFullyExplored_(const RobotMap& map) const {
		double relEntropy = map.getRelativeEntropy();   //获取地图的相对熵
		if (relEntropy >= 0.99)  //相对熵很高 直接返回flase
			return false;

		double relFrontierSize = map.getFrontier().size() / (double)map.getNCells(); //getFrontier() 是一个遍历地图并提取“可探索边界格子”的函数
	...
	bool PlannerServer::planServiceCallback_(
		dmce_msgs::GetPlan::Request& request,  //接受请求
		dmce_msgs::GetPlan::Response& response)  //返回路径计划作为相应
