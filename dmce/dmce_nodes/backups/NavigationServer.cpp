...
namespace dmce {
		...
		: NodeServer(nh, robotId), actionClient_("Pathfinding", false)
	{
		...
		omega_ = 0.0;
		use_cmd_vel_ = false;
		manual_position_ = startingPosition;
		theta_ = 0;

		cmdVelSub_ = nodeHandle_.subscribe("cmd_vel", 10,   // ✅ 新增
                    &NavigationServer::cmdVelCallback_, this);
	...
	bool NavigationServer::navigateToGoal_(const ros::Duration& timeStep) {
		if (!navigation_->isReady()){
			// logWarn("navigateToGoal_", "⚠️ Navigation not ready, skipping moveRobotToGoal()");
			return true;
		}
	...
	void NavigationServer::publishRobotPosition_(const pos_t& pos) {
		dmce_msgs::RobotPosition msg;
		msg.robotId = robotId_;
		msg.x_position = pos.x();
		msg.y_position = pos.y();
		positionPublisher_.publish(msg);
	}
	...
	void NavigationServer::cmdVelCallback_(const geometry_msgs::Twist& msg) {
		vx_ = msg.linear.x;
		vy_ = msg.linear.y;
		omega_ = msg.angular.z;

		// 如果控制量足够小，就认为用户松开键，回到自动模式
		if (fabs(vx_) < 1e-3 && fabs(vy_) < 1e-3 && fabs(omega_) < 1e-3)
			use_cmd_vel_ = false;
		else
			use_cmd_vel_ = true;
	}
