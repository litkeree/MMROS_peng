...
namespace dmce {
	...
	dmce_msgs::FrontierClusters FrontierClustering::getMessage() const {  //定义一个名为 getMessage 的常量成员函数，它返回一个 dmce_msgs::FrontierClusters 类型的消息对象 
		dmce_msgs::FrontierClusters msg;  //创建一个空的 FrontierClusters 消息对象 msg，用于填充内容
		msg.nClusters = getNClusters();  //将当前聚类的数量写入消息字段 nClusters
		msg.mapResolution = resolution_;  //将当前地图分辨率赋值给消息中的 mapResolution 字段
		msg.mapOffset_x = mapOffset_.x();  //将地图原点的偏移量（x 和 y）赋值给消息
		msg.mapOffset_y = mapOffset_.y();
		for (unsigned int i = 0; i < getNClusters(); ++i) {   //遍历每个聚类
			msg.clusterCentroids_x.push_back(clusterCentroids_[i].x());   //把每个聚类的质心坐标（x、y）和大小写入消息字段
			msg.clusterCentroids_y.push_back(clusterCentroids_[i].y());    
			msg.clusterSizes.push_back(clusterSizes_[i]);
		}
		for (const auto& element : frontierCells_) {
			msg.cellIndices_x.push_back(element.first.x());  //element.first 表示格子坐标，element.second 是它的聚类编号
			msg.cellIndices_y.push_back(element.first.y());
			msg.cellClusterAssignment.push_back(element.second);
		}
		return msg;   //将内部的聚类数据（包括聚类数、质心、前沿格子、聚类编号等）封装成一个标准 ROS 消息 dmce_msgs::FrontierClusters，用于后续处理（
	}

	void FrontierClustering::fromMessage(const dmce_msgs::FrontierClusters& msg) {
		reset();
