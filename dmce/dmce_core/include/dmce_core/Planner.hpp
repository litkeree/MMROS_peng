...
namespace dmce {
	...
	public:
		...
		std::pair<bool, plan_t> getLatestPlan() {   //返回一个布尔值和路径plan_t
			if (!isReady_())
				return std::make_pair(false, plan_t{});  //返回false 路径为空
		 ...
		 */
		virtual void peerPlanCallback(const dmce_msgs::RobotPlan& msg) {
		 ...
		 */
		virtual void frontierClusterCallback(const dmce_msgs::FrontierClusters& msg) {
